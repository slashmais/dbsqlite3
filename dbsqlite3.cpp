...
DB_ID_TYPE DBsqlite3::new_id(const std::string &tname)
{
	...
	//else telluser("ERROR: ", sLastError);
	return id;
}
...
bool DBsqlite3::isvalid_id(const std::string &id_name, DB_ID_TYPE id)
{
	...
	sSQL=spf("SELECT id, del FROM IDS WHERE name = ", SQLSafeQ(id_name));
	ExecSQL(&RS, sSQL);
	if (NoError(this))
	{
...
bool DBsqlite3::MakeTable(const std::string sT, const std::string sF, const std::string sC)
{
	DBResult RS;
	std::string sSQL;
	sSQL=spf("CREATE TABLE IF NOT EXISTS ", sT.c_str(), "(", sF.c_str(), ") ", sC);
	ExecSQL(&RS, sSQL);
...
{
	if (!b) tellerror(path_name(pdb->sDB), ": [", pdb->GetLastErrorRC(), "] ", pdb->GetLastError());
	return b;
}
